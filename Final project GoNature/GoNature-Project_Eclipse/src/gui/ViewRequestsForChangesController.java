...
public class ViewRequestsForChangesController implements Initializable {
	    ...
	    new Thread(() -> {
	        try {
	            ...
	            if (servermsg.getFlag()) {
	                ...
	                Platform.runLater(() -> {
	                    Alerts somethingWentWrong = new Alerts(Alerts.AlertType.INFORMATION, "INFORMATION", "",
	...
	private void getParkOldGap() {
		...
		this.oldGap = parkNamesToNumbers.get(CurWorker.getWorksAtPark()).getGap();
	...
	private void confirmRequestBtn(ActionEvent event) {
		...
		if (selectedRequest != null) {
			...
			ClientServerMessage<ChangeRequest> messageForServer = new ClientServerMessage<>(selectedRequest,
					Operation.PATCH_PARK_PARAMETERS);
			ClientUI.clientControllerInstance.sendMessageToServer(messageForServer);
			...
			Alerts somethingWentWrong = new Alerts(Alerts.AlertType.ERROR, "ERROR", "",
					"You did not select a change request, please select again.");
			somethingWentWrong.showAndWait();
		}
	}
	...
	/**
	 * Common method to handle both confirm and cancel operations for change
	 * requests.
	 */
	private void handleRequestChange(String status) {
		...
		if (selectedRequest != null) {
			...
			ClientServerMessage<ChangeRequest> messageForServer = new ClientServerMessage<>(selectedRequest,
					Operation.PATCH_CHANGE_REQUEST_STATUS);
			ClientUI.clientControllerInstance.sendMessageToServer(messageForServer);
	...
	/**
	 * Refreshes the list of change requests by fetching updated data from the
	 * server.
	 */
