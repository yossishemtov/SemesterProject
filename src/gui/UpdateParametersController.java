	    ...
	    if (validateInput()) {
	        try {
	            ...
	            // Construct the ChangeRequest object
	                ...
	                0, // Assuming an 'id' is not needed or will be auto-generated by the database
	                parkData.getName(),
	                parkNumber,
	                newMaxVisitors,
	                newAllowedGap,
	                newStayTime,
	                "WAITING_FOR_APPROVAL" // Assuming a new request starts with a status indicating it's waiting for approval
	            );
	                
	                ClientServerMessage<?> requestForServer = new ClientServerMessage<>(request, Operation. POST_NEW_CHANGE_REQUEST);
	    			ClientUI.clientControllerInstance.sendMessageToServer(requestForServer);
	    			if (ClientController.data.getFlag())
	    			{
	    	            System.out.println("in get flag");

	    				Alerts infoalert = new Alerts(Alerts.AlertType.INFORMATION, "INFORMATION","", "A change request was successfully received");
	    				infoalert.showAndWait();
	    			}
	    			else {
	    				Alerts somethingWentWrong = new Alerts(Alerts.AlertType.ERROR, "ERROR","", "A change request was not successful");
